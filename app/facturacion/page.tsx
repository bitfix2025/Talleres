...
export default function FacturacionPage(){
 ...
 return <main className="min-h-screen bg-gradient-to-br from-[#eefaf4] via-white to-[#f4f7ff] p-4 text-[#17201b] md:p-8"><div className="mx-auto max-w-7xl">
  <header className="mb-6 overflow-hidden rounded-3xl border bg-white/90 p-6 shadow-sm"><div className="flex flex-wrap items-center justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[.2em] text-green-700">Administración</p><h1 className="text-3xl font-black">Facturación</h1><p className="mt-1 text-sm text-gray-500">Lo facturado y la ganancia real de reparaciones y ventas.</p></div><div className="flex gap-2"><button onClick={()=>router.push("/")} className="rounded-xl border bg-white px-4 py-2.5 text-sm font-bold"><ArrowLeft size={16} className="mr-2 inline"/>Volver</button><button onClick={cargar} className="rounded-xl border bg-white px-4 py-2.5 text-sm font-bold"><RefreshCw size={16} className="mr-2 inline"/>Actualizar</button></div></div><div className="mt-5 grid gap-3 sm:grid-cols-3"><div className="rounded-2xl bg-[#e8f8f0] p-4"><p className="text-xs font-bold uppercase tracking-wide text-green-700">Reparaciones entregadas</p><p className="mt-1 text-2xl font-black">{reparacionesFacturadas.length}</p></div><div className="rounded-2xl bg-[#eef2ff] p-4"><p className="text-xs font-bold uppercase tracking-wide text-indigo-700">Ventas completadas</p><p className="mt-1 text-2xl font-black">{ventas.length}</p></div><div className="rounded-2xl bg-[#fff5df] p-4"><p className="text-xs font-bold uppercase tracking-wide text-amber-700">Actividad</p><p className="mt-1 text-sm font-bold">Datos actualizados en tiempo real</p></div></div></header>
  ...
  <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
   {[[DollarSign,"Facturado hoy",factHoy],[TrendingUp,"Ganancia hoy",ganHoy],[DollarSign,"Facturado este mes",factMes],[TrendingUp,"Ganancia este mes",ganMes]].map(([Icon,label,value]:any)=><div className="rounded-2xl border bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md" key={label}><div className="mb-3 flex h-10 w-10 items-center justify-center rounded-xl bg-green-50 text-green-700"><Icon size={19}/></div><p className="text-sm font-semibold text-gray-500">{label}</p><p className="mt-1 text-2xl font-black">{money(value)}</p></div>)}
  </div>
