...
export async function POST(request: NextRequest) {
  try {
    ...
    if (perfilError || perfil?.rol !== "administrador" || !perfil.activo) return NextResponse.json({ error: "Solo un administrador activo puede crear usuarios." }, { status: 403 });
    const body = await request.json();
    const nombre = String(body.nombre ?? "").trim();
    const email = String(body.email ?? "").trim().toLowerCase();
    const password = String(body.password ?? "");
    const rol = String(body.rol ?? "");
    if (!nombre || !email || !password || !["tecnico","recepcion"].includes(rol)) return NextResponse.json({ error: "Completá nombre, correo, contraseña y rol." }, { status: 400 });
    ...
    const { error: insertError } = await admin.from("perfiles").insert({ id: created.user.id, nombre, email, rol, activo: true });
