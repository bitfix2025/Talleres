...
export default function ComprobanteRecepcion(){
 ...
 useEffect(()=>{(async()=>{const r=await supabase.from("ordenes_reparacion").select("id,estado,falla_reportada,observaciones,created_at,clientes(nombre,telefono),equipos(marca,modelo,imei,numero_serie,color,capacidad,bateria_porcentaje)").eq("id",id).maybeSingle();if(r.error||!r.data){setError(r.error?.message||"No se encontró la orden.");setCargando(false);return}const o=r.data as any;setOrden({...o,cliente:Array.isArray(o.clientes)?o.clientes[0]||null:o.clientes||null,equipo:Array.isArray(o.equipos)?o.equipos[0]||null:o.equipos||null});const c=await supabase.from("checklist_reparacion").select("categoria,prueba,estado,observacion").eq("orden_id",id).eq("momento","ENTRADA").order("orden_prueba");if(!c.error)setCheck((c.data||[]) as Fila[]);setCargando(false)})()},[id]);
 ...
 return <main className="min-h-screen bg-gray-100 p-4 print:bg-white print:p-0"><div className="mx-auto mb-4 flex max-w-[850px] gap-2 print:hidden"><button onClick={()=>router.push("/reparaciones/"+id)} className="rounded-xl border bg-white px-4 py-2 text-sm font-bold"><ArrowLeft size={16} className="mr-2 inline"/>Volver</button><button onClick={()=>router.push("/")} className="rounded-xl border bg-white px-4 py-2 text-sm font-bold"><Home size={16} className="mr-2 inline"/>Inicio</button><button onClick={()=>window.print()} className="rounded-xl bg-green-600 px-4 py-2 text-sm font-bold text-white"><Printer size={16} className="mr-2 inline"/>Imprimir</button></div>
 <article className="mx-auto max-w-[850px] bg-white p-7 text-[11px] leading-4 text-gray-900 shadow-sm print:max-w-none print:p-6 print:shadow-none">
  <header className="border-b-2 border-green-600 pb-4"><div className="flex items-start justify-between gap-5"><div><div className="text-3xl font-black tracking-tight text-green-700">BITFIX</div><p className="mt-1 font-bold uppercase tracking-widest">Orden de reparación</p><p className="text-gray-500">Comprobante de recepción</p></div><div className="text-right"><p className="text-[10px] uppercase text-gray-500">N° de orden</p><p className="text-2xl font-black">#{String(orden.id).padStart(5,"0")}</p><p>{fecha(orden.created_at)}</p></div></div></header>
  <section className="mt-4 grid grid-cols-2 gap-4"><Box title="CLIENTE"><b>{orden.cliente?.nombre||"-"}</b><br/>Teléfono: {orden.cliente?.telefono||"-"}</Box><Box title="EQUIPO"><b>{[orden.equipo?.marca,orden.equipo?.modelo].filter(Boolean).join(" ")||"-"}</b><br/>IMEI: {orden.equipo?.imei||"-"}<br/>Serie: {orden.equipo?.numero_serie||"-"}<br/>Color: {orden.equipo?.color||"-"} · {orden.equipo?.capacidad||"-"} · Batería: {orden.equipo?.bateria_porcentaje!=null?orden.equipo.bateria_porcentaje+"%":"-"}</Box></section>
