...
type Orden = { contraseña_equipo?: string | null; id:number; taller_id:number|null; cliente_id:number|null; equipo_id:number|null; estado:string|null; falla_reportada:string|null; observaciones:string|null; created_at:string|null; presupuesto_mano_obra:number|null; cliente:Cliente|null; equipo:Equipo|null };
...
export default function ReparacionDetallePage(){
  ...
  const cargar=async()=>{
    ...
    const {data,error:e}=await supabase.from("ordenes_reparacion").select(`id,taller_id,cliente_id,equipo_id,estado,falla_reportada,observaciones,created_at,presupuesto_mano_obra,contraseña_equipo,clientes(nombre,dni,telefono),equipos(marca,modelo,imei,numero_serie,color,capacidad,bateria_porcentaje)`).eq("id",ordenId).maybeSingle();
    ...
    const obs=data.observaciones||""; const dm=obs.match(/Diagnóstico:\s*([\s\S]*?)(?:\n\nNotas técnicas:|$)/i);const nm=obs.match(/Notas técnicas:\s*([\s\S]*)$/i);setDiagnostico(dm?.[1]?.trim()||"");setNotas(nm?.[1]?.trim()||(dm?"":obs));setManoObra(data.presupuesto_mano_obra!=null?String(data.presupuesto_mano_obra):""); setContrasenaEquipo(data.contraseña_equipo||"");
  ...
  if(cargando)<div className="hidden print:block print:p-0"><div className="mx-auto w-[90mm] border-2 border-black p-3 text-black"><div className="text-center text-xl font-black">BITFIX</div><div className="mt-1 border-y border-black py-1 text-center text-lg font-black">ORDEN #{String(orden?.id||0).padStart(5,"0")}</div><div className="mt-2 text-[11px] leading-4"><b>CLIENTE:</b> {orden?.cliente?.nombre||"-"}<br/><b>EQUIPO:</b> {[orden?.equipo?.marca,orden?.equipo?.modelo].filter(Boolean).join(" ")||"-"}<br/><b>IMEI:</b> {orden?.equipo?.imei||"-"}<br/><b>CLAVE:</b> {contrasenaEquipo||"-"}<br/><b>FECHA:</b> {fecha(orden?.created_at||null)}</div><div className="mt-2 border-t border-black pt-2 text-[11px]"><b>TRABAJO / PROBLEMA</b><div className="mt-1 min-h-[25mm] whitespace-pre-wrap">{orden?.falla_reportada||"-"}</div></div></div></div>\n  return <main className="min-h-screen bg-[#f5f6f8] flex items-center justify-center"><Loader2 size={32} className="animate-spin"/></main>;
