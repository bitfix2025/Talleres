...
export default function ReparacionDetallePage(){
  ...
  const abrirSeccion=(s:"diagnostico"|"presupuesto"|"reparacion")=>{setSeccion(s);setTimeout(()=>document.getElementById(s)?.scrollIntoView({behavior:"smooth",block:"start"}),0)};
  return <main className="min-h-screen bg-[#f5f6f8] text-gray-900"><div className="mx-auto max-w-[1200px] p-4 md:p-6">
    ...
    <div className="mt-6 space-y-4">
      <section className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
        ...
        <div className="mt-4 rounded-xl bg-gray-50 p-4"><p className="text-xs font-bold uppercase text-gray-400">Falla reportada</p><p className="mt-1 whitespace-pre-wrap text-sm">{orden.falla_reportada||"Sin falla registrada."}</p></div>
      </section>
      <section id="diagnostico" className="scroll-mt-5 rounded-2xl border border-gray-200 bg-white shadow-sm">
        <button onClick={()=>setSeccion(seccion==="diagnostico"?"reparacion":"diagnostico")} className="flex w-full items-center justify-between p-6 text-left"><div><p className="text-xs font-bold uppercase text-gray-400">Paso 1</p><h2 className="text-xl font-bold">Diagnóstico</h2><p className="text-sm text-gray-500">Registrá qué encontraste y las notas técnicas.</p></div>{seccion==="diagnostico"?<ChevronUp/>:<ChevronDown/>}</button>
        {seccion==="diagnostico"&&<div className="border-t p-6"><textarea value={diagnostico} onChange={e=>setDiagnostico(e.target.value)} rows={4} className="w-full rounded-xl border border-gray-200 bg-gray-50 p-4 text-sm" placeholder="Qué se encontró..."/><textarea value={notas} onChange={e=>setNotas(e.target.value)} rows={3} className="mt-4 w-full rounded-xl border border-gray-200 bg-gray-50 p-4 text-sm" placeholder="Notas técnicas..."/><div className="mt-4 flex justify-end"><button disabled={guardando} onClick={guardarNotas} className="rounded-xl bg-black px-5 py-3 text-sm font-bold text-white">Guardar diagnóstico</button></div></div>}
      </section>
      <section id="presupuesto" className="scroll-mt-5 rounded-2xl border border-gray-200 bg-white shadow-sm">
        <button onClick={()=>setSeccion(seccion==="presupuesto"?"reparacion":"presupuesto")} className="flex w-full items-center justify-between p-6 text-left"><div><p className="text-xs font-bold uppercase text-gray-400">Paso 2</p><h2 className="text-xl font-bold">Presupuesto</h2><p className="text-sm text-gray-500">Agregá repuestos y mano de obra.</p></div>{seccion==="presupuesto"?<ChevronUp/>:<ChevronDown/>}</button>
        {seccion==="presupuesto"&&<div className="border-t p-6 space-y-5">
          <div className="grid gap-3 md:grid-cols-[1fr_110px_140px_auto]"><div><input value={busqueda} onChange={e=>setBusqueda(e.target.value)} placeholder="Buscar repuesto..." className="h-11 w-full rounded-xl border px-4 text-sm"/><select value={productoId} onChange={e=>setProductoId(e.target.value)} className="mt-2 h-11 w-full rounded-xl border px-3 text-sm"><option value="">Seleccionar repuesto</option>{disponibles.map(p=><option key={p.id} value={p.id}>{p.nombre} · stock {p.stock_actual}</option>)}</select></div><input type="number" min="1" value={cantidad} onChange={e=>setCantidad(e.target.value)} className="h-11 rounded-xl border px-4"/><input type="number" min="0" step=".01" value={precioVenta} onChange={e=>setPrecioVenta(e.target.value)} className="h-11 rounded-xl border px-4"/><button disabled={guardando} onClick={agregarRepuesto} className="h-11 rounded-xl bg-black px-5 text-sm font-bold text-white"><Plus size={17} className="inline mr-1"/>Agregar</button></div>
          ...
          <div className="grid gap-4 md:grid-cols-[1fr_250px]"><div><label className="text-xs font-bold uppercase text-gray-400">Mano de obra</label><input type="number" min="0" step=".01" value={manoObra} onChange={e=>setManoObra(e.target.value)} className="mt-2 h-11 w-full rounded-xl border px-4"/></div><div className="rounded-xl bg-gray-950 p-5 text-white"><p className="text-xs text-gray-400">TOTAL</p><p className="text-3xl font-bold">{dinero(total)}</p></div></div>
          <div className="flex justify-end"><button disabled={guardando} onClick={()=>guardarPresupuesto(true)} className="rounded-xl bg-black px-6 py-3 text-sm font-bold text-white">Guardar y enviar aprobación <ArrowRight size={16} className="inline ml-1"/></button></div>
        </div>}
      </section>
      <section id="reparacion" className="scroll-mt-5 rounded-2xl border border-gray-200 bg-white shadow-sm">
        <button onClick={()=>setSeccion(seccion==="reparacion"?"diagnostico":"reparacion")} className="flex w-full items-center justify-between p-6 text-left"><div><p className="text-xs font-bold uppercase text-gray-400">Paso 3</p><h2 className="text-xl font-bold">Reparación y entrega</h2><p className="text-sm text-gray-500">Cuando el presupuesto esté aprobado, continuás desde acá.</p></div>{seccion==="reparacion"?<ChevronUp/>:<ChevronDown/>}</button>
        {seccion==="reparacion"&&<div className="border-t p-6"><div className="grid gap-3 md:grid-cols-3"><button onClick={()=>router.push(`/reparaciones/${orden.id}/checklist`)} className="rounded-xl border p-4 text-left font-bold">Checklist</button><button onClick={()=>router.push(`/reparaciones/${orden.id}/fotos`)} className="rounded-xl border p-4 text-left font-bold">Fotos</button><button onClick={avanzar} disabled={!siguiente||guardando} className="rounded-xl bg-black p-4 text-left font-bold text-white">Avanzar reparación</button></div></div>}
      </section>
    </div>
