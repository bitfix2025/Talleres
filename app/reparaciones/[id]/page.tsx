...
type Tecnico = { id:string; nombre:string|null; rol:string|null; activo:boolean };
...
export default function ReparacionDetallePage(){
  ...
  const [comprobanteRetiro,setComprobanteRetiro]=useState<"COMPROBANTE"|"TICKET"|"FACTURA">("COMPROBANTE");
  ...
  const agregarRepuesto=async()=>{if(!orden||guardando)return;setError("");setMensaje("");
    ...
    if(!Number.isInteger(q)||q<=0)return setError("La cantidad debe ser un número entero mayor a 0.");
    if(!Number.isFinite(precio)||precio<0)return setError("Ingresá un precio de venta válido.");
    ...
    try{
      ...
      if(productoBD.error)throw new Error(productoBD.error.message);
      if(!productoBD.data)throw new Error("No se encontró el repuesto seleccionado.");
      const nombreProducto=String(productoBD.data.nombre||p.nombre||"").trim();
      const costoProducto=Number(productoBD.data.costo??p.costo??0);
      if(!nombreProducto)throw new Error("El repuesto no tiene nombre en el inventario.");
  ...
  const registrarPago=async()=>{if(!orden||guardando)return;const monto=Number(montoPago);if(!Number.isFinite(monto)||monto<=0)return setError("Ingresá un monto de pago válido.");if(monto>saldo+0.01)return setError(`El pago supera el saldo pendiente de ${dinero(saldo)}.`);setGuardando(true);setError("");setMensaje("");
    try{
  ...
  return <main className="min-h-screen bg-[#f3f7f5] text-gray-900"><div className="mx-auto max-w-[1200px] p-4 md:p-6">
    ...
    {(normalizar(orden.estado)==="LISTO PARA ENTREGAR"||normalizar(orden.estado)==="ENTREGADO")&&<section className="mt-5 rounded-2xl border border-green-200 bg-white p-6 shadow-sm">
      <div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-wide text-[#15803d]">Retiro del equipo</p><h2 className="mt-1 text-xl font-black">Documentación de entrega</h2><p className="text-sm text-gray-500">Elegí qué comprobante necesita el cliente al retirar.</p></div><FileText className="text-[#16a34a]" size={24}/></div>
      <div className="mt-5 grid gap-3 md:grid-cols-3">
        {([["COMPROBANTE","Comprobante de reparación","Entrega con total, abonado y saldo."],["TICKET","Ticket / comprobante","Para entregar un comprobante simple de la reparación."],["FACTURA","Factura","Indica que el cliente solicita factura fiscal."] ] as const).map(([valor,titulo,descripcion])=><button key={valor} type="button" onClick={()=>setComprobanteRetiro(valor)} className={`rounded-xl border p-4 text-left transition ${comprobanteRetiro===valor?"border-[#16a34a] bg-green-50 ring-2 ring-green-100":"border-gray-200 bg-white hover:border-green-300"}`}><div className="flex items-center gap-3"><span className={`flex h-5 w-5 items-center justify-center rounded-full border-2 ${comprobanteRetiro===valor?"border-[#16a34a]":"border-gray-300"}`}>{comprobanteRetiro===valor&&<span className="h-2.5 w-2.5 rounded-full bg-[#16a34a]"/>}</span><span className="font-black">{titulo}</span></div><p className="mt-2 pl-8 text-xs text-gray-500">{descripcion}</p></button>)}
      </div>
      {comprobanteRetiro==="FACTURA"&&<div className="mt-4 rounded-xl border border-blue-200 bg-blue-50 p-4 text-sm text-blue-800"><b>Factura:</b> esta opción registra la solicitud de factura. La emisión fiscal deberá realizarse mediante la integración de facturación autorizada.</div>}
      <div className="mt-5 flex flex-wrap gap-3"><button onClick={()=>router.push(`/reparaciones/${orden.id}/comprobante?tipo=${comprobanteRetiro.toLowerCase()}`)} className="inline-flex items-center gap-2 rounded-xl bg-[#16a34a] px-5 py-3 text-sm font-bold text-white"><Printer size={17}/> Imprimir comprobante de entrega</button></div>
    </section>}
