...
export default function ReparacionDetallePage(){
  ...
  const cambiarEstado=async(db:string)=>{if(!orden||guardando)return;
    const permitidos:Record<string,string|undefined>={"RECIBIDO":"DIAGNÓSTICO","DIAGNÓSTICO":"PRESUPUESTADO","PRESUPUESTADO":"ESPERANDO APROBACIÓN","ESPERANDO APROBACIÓN":"APROBADO","APROBADO":"EN REPARACIÓN","EN REPARACIÓN":"LISTO PARA ENTREGAR","LISTO PARA ENTREGAR":"ENTREGADO"};
    const esperado=permitidos[normalizar(orden.estado)];
    if(db!==esperado){setError(`No se puede pasar de ${orden.estado||"-"} a ${db} desde esta acción.`);return;}
